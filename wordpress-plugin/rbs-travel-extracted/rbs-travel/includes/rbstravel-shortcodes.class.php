...
class RBS_TRAVEL_Shortcodes {
    ...
    public function featured_travels_shortcode($atts) {
        ...
        // Parse IDs (Travel Compositor Idea IDs)
        ...
        if (empty($ids)) {
            if (current_user_can('edit_posts')) {
                return '<div style="padding: 20px; background: #fff3cd; border: 1px solid #ffc107; border-radius: 8px; margin: 20px 0;">
                    ...
                    <small>Gebruik: <code>[uitgelichte_reizen ids="123,456,789"]</code> (Travel Compositor Idea IDs)</small>
                </div>';
            }
            return '';
        }
        
        // Search by Travel Compositor Idea ID (stored in meta key 'travel_id')
        $travels = array();
        ...
        // Fallback to WordPress post IDs if no travels found by Idea ID
        if (empty($travels)) {
