<?php
add_filter ('woocommerce_add_to_cart_redirect', 'travellers_redirect_to_checkout');

function travellers_redirect_to_checkout() {
    global $woocommerce;
    $checkout_url = wc_get_checkout_url();
    return $checkout_url;
}

function travellers_pricing_signup_url(){
	global $woocommerce, $post;
    $checkout_url = wc_get_checkout_url().'?add-to-cart='.$post->ID;
    return $checkout_url;
}
...
if ( ! function_exists( 'woocommerce_form_field' ) ) {
	...
	function woocommerce_form_field( $key, $args, $value = null ) {
		...
		if ( $args['required'] ) {
			...
			$required = ' <abbr class="required" title="' . esc_attr__( 'required', 'travellers'  ) . '">*</abbr>';
		} else {
			$required = '';
		}
			...
			case 'country' :
				...
				if ( 1 === sizeof( $countries ) ) {
					...
					$field = '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" ' . $args['autocomplete'] . ' class="country_to_state country_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) .'" ' . implode( ' ', $custom_attributes ) . '>'
							. '<option value="">'.esc_attr(__( 'Select a country&hellip;', 'travellers' )) .'</option>';
					...
					$field .= '<noscript><input type="submit" name="woocommerce_checkout_update_totals" value="' . esc_attr__( 'Update country', 'travellers' ) . '" /></noscript>';
			...
			case 'state' :
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) .'" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '" ' . $args['autocomplete'] . '>
						<option value="">'.esc_attr(__( 'Select a state&hellip;', 'travellers' )) .'</option>';
					...
					foreach ( $args['options'] as $option_key => $option_text ) {
						if ( '' === $option_key ) {
							...
							if ( empty( $args['placeholder'] ) ) {
								$args['placeholder'] = $option_text ? $option_text : __( 'Choose an option', 'travellers' );
							}
