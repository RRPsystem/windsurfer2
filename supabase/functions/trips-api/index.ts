// Supabase Edge Function: trips-api
// Returns active trips for a brand, enriched from Travel Compositor Idea /info API
...
serve(async (req) => {
  ...
  try {
    ...
    // Init Supabase client with service role (on server)
    ...
    // 1) Fetch active feed config for this brand
    ...
    // 2) Fetch active idea_ids for this brand
    ...
    // Travel Compositor API base + auth token from env
    ...
    // 3) For each idea_id, call Travel Compositor /info
    const items: any[] = [];
...
function mapIdeaVoToItem(ideaVo: any, micrositeId: string, ideaRow: any) {
  ...
  // Bestemming uit destinations of itinerary
  let destination = '';
  ...
  // Duur uit counters (hotelNights of closedTours)
  ...
  // Link naar detail (kan later naar eigen route wijzen)
