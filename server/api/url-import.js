// server/api/url-import.js
// URL booking import with Cheerio + OpenAI extraction
// Supports all Travel Compositor fields

const axios = require('axios');
const cheerio = require('cheerio');

// Comprehensive extraction prompt with all TC fields
const EXTRACTION_PROMPT = `Je bent een expert in het extraheren van complete reisgegevens uit booking websites.

Extraheer ALLE beschikbare informatie en geef het terug als JSON in dit formaat:

{
  "name": "Reis naam/titel",
  "title": "Reis titel",
  "description": "Volledige beschrijving",
  "intro": "Korte intro tekst",
  "destination": "Bestemming/land",
  "country": "Land",
  "region": "Regio",
  "city": "Stad",
  "bookingReference": "PNR/booking nummer",
  "duration": aantal_dagen,
  "travelers": [
    {
      "name": "Volledige naam",
      "type": "adult/child/infant",
      "age": leeftijd_optioneel
    }
  ],
  "flights": [
    {
      "from": "Vertrek luchthaven (code)",
      "fromName": "Volledige naam vertrek",
      "to": "Aankomst luchthaven (code)",
      "toName": "Volledige naam aankomst",
      "date": "YYYY-MM-DD",
      "time": "HH:MM",
      "arrivalDate": "YYYY-MM-DD",
      "arrivalTime": "HH:MM",
      "flightNumber": "Vluchtnummer",
      "airline": "Luchtvaartmaatschappij",
      "class": "Economy/Business/First",
      "duration": "Vliegduur in minuten"
    }
  ],
  "hotel": {
    "name": "Hotel naam",
    "address": "Volledig adres",
    "city": "Stad",
    "country": "Land",
    "checkIn": "YYYY-MM-DD",
    "checkOut": "YYYY-MM-DD",
    "roomType": "Kamer type",
    "nights": aantal_nachten,
    "board": "All-inclusive/Half board/etc",
    "stars": aantal_sterren,
    "amenities": ["faciliteit1", "faciliteit2"]
  },
  "days": [
    {
      "day": dag_nummer,
      "title": "Dag titel",
      "description": "Dag beschrijving",
      "activities": [
        {
          "name": "Activiteit naam",
          "description": "Beschrijving",
          "time": "HH:MM optioneel",
          "duration": minuten_optioneel
        }
      ],
      "accommodation": "Hotel naam voor deze dag",
      "meals": ["breakfast", "lunch", "dinner"],
      "transport": [
        {
          "type": "flight/bus/train/car",
          "from": "Van",
          "to": "Naar",
          "time": "HH:MM"
        }
      ]
    }
  ],
  "price": {
    "total": bedrag_als_nummer,
    "currency": "EUR/USD/etc",
    "perPerson": bedrag_per_persoon_optioneel,
    "deposit": aanbetaling_optioneel,
    "included": ["wat_is_inbegrepen"],
    "excluded": ["wat_is_niet_inbegrepen"]
  },
  "dates": {
    "departure": "YYYY-MM-DD",
    "return": "YYYY-MM-DD",
    "bookingDate": "YYYY-MM-DD optioneel"
  },
  "inclusions": [
    "Wat is inbegrepen in de reis"
  ],
  "exclusions": [
    "Wat is niet inbegrepen"
  ],
  "highlights": [
    "Hoogtepunten van de reis"
  ],
  "itinerary": {
    "summary": "Samenvatting van de route",
    "map": "URL naar kaart optioneel"
  },
  "images": [
    {
      "url": "Image URL",
      "caption": "Beschrijving",
      "type": "main/gallery/day"
    }
  ],
  "contacts": {
    "agency": "Reisbureau naam",
    "phone": "Telefoonnummer",
    "email": "Email",
    "emergencyContact": "Noodcontact"
  },
  "documents": {
    "visa": "Visum info",
    "passport": "Paspoort vereisten",
    "insurance": "Verzekering info",
    "vaccination": "Vaccinatie vereisten"
  },
  "cancellationPolicy": "Annuleringsvoorwaarden",
  "importantInfo": [
    "Belangrijke informatie"
  ]
}

Als informatie ontbreekt, gebruik null of lege array. Geef ALLEEN de JSON terug, geen extra tekst.`;

async function extractFromHTML(html, url) {
  const $ = cheerio.load(html);
  
  // Basic extraction with Cheerio (fallback/enhancement)
  const extracted = {
    title: $('h1').first().text().trim() || $('title').text().trim(),
    description: $('meta[name="description"]').attr('content') || '',
    images: []
  };
  
  // Extract images
  $('img').each((i, elem) => {
    const src = $(elem).attr('src');
    const alt = $(elem).attr('alt');
    if (src && !src.includes('logo') && !src.includes('icon')) {
      extracted.images.push({
        url: src.startsWith('http') ? src : new URL(src, url).href,
        caption: alt || '',
        type: i === 0 ? 'main' : 'gallery'
      });
    }
  });
  
  return extracted;
}

async function extractWithAI(html, cheerioData) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) {
    throw new Error('OPENAI_API_KEY not configured');
  }

  // Combine HTML text with cheerio extracted data
  const $ = cheerio.load(html);
  const textContent = $('body').text().replace(/\s+/g, ' ').trim();
  
  const prompt = `URL: ${cheerioData.url || 'N/A'}
Titel: ${cheerioData.title || 'N/A'}
Beschrijving: ${cheerioData.description || 'N/A'}

HTML Content:
${textContent.substring(0, 15000)}

${EXTRACTION_PROMPT}`;

  const response = await fetch('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${apiKey}`
    },
    body: JSON.stringify({
      model: 'gpt-4o-mini',
      messages: [
        {
          role: 'system',
          content: 'Je bent een expert in het extraheren van complete reisgegevens. Geef altijd valide JSON terug.'
        },
        {
          role: 'user',
          content: prompt
        }
      ],
      temperature: 0.1,
      response_format: { type: "json_object" }
    })
  });

  if (!response.ok) {
    const error = await response.text();
    throw new Error(`OpenAI API error: ${error}`);
  }

  const data = await response.json();
  const content = data.choices[0].message.content;
  
  const extracted = JSON.parse(content);
  
  // Merge with cheerio data (AI takes precedence, but cheerio fills gaps)
  if (!extracted.images || extracted.images.length === 0) {
    extracted.images = cheerioData.images;
  }
  
  return extracted;
}

async function handler(req, res) {
  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Method not allowed' });
  }

  const { url } = req.body;

  if (!url) {
    return res.status(400).json({ error: 'URL is required' });
  }

  try {
    console.log('[URLImport] Fetching URL:', url);

    // Fetch HTML
    const response = await axios.get(url, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
      },
      timeout: 15000,
      maxRedirects: 5
    });

    const html = response.data;
    console.log('[URLImport] HTML fetched, length:', html.length);

    // Extract with Cheerio
    const cheerioData = await extractFromHTML(html, url);
    cheerioData.url = url;

    console.log('[URLImport] Cheerio extraction done');

    // Extract with AI
    const travelData = await extractWithAI(html, cheerioData);

    console.log('[URLImport] AI extraction successful');

    return res.json({
      success: true,
      data: travelData,
      meta: {
        url: url,
        extractedAt: new Date().toISOString(),
        method: 'cheerio + openai'
      }
    });

  } catch (error) {
    console.error('[URLImport] Error:', error);
    
    if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') {
      return res.status(400).json({
        error: 'URL niet bereikbaar',
        detail: 'De opgegeven URL kon niet worden geladen. Controleer of de URL correct is.'
      });
    }
    
    if (error.response?.status === 403 || error.response?.status === 401) {
      return res.status(400).json({
        error: 'Toegang geweigerd',
        detail: 'De website blokkeert automatische toegang. Probeer een andere URL of gebruik PDF upload.'
      });
    }

    return res.status(500).json({
      error: 'URL import failed',
      detail: error.message
    });
  }
}

module.exports = handler;
